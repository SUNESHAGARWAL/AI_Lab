...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    async with postgres_checkpointer(settings.database_url) as checkpointer:
        # No real retrieval adapter yet — packages/retrieval lands separately (ADR
        # 0001). An empty-corpus FakeRetriever keeps the graph runnable in the
        # meantime; swap this for the real adapter without touching graph/node code.
